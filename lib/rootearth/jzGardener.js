...
function jzGardener(){
    ...
    this.plant = function( strFileName ){
        ...
        let strDirName = orig.path.dirname( strFileName ),
            seedlingBundle = this.getSeedlingBundle( strFileName );
        cuttage( seedlingBundle, strDirName );
        this.gpList.push( strFileName );
    };

    /* 取得幼苗枝幹
     * @param {String} fileName : 生長點 JSON 檔路徑
     * @return {JSON} seedlingBundle : 幼苗枝幹
     */
    this.getSeedlingBundle = function( strFileName ){
    ...
    function cuttage( arrSeedlingBundle, strDirName ){
        ...
        while( p < len ){
            ...
            isOrigClass = item.classify === 'orig';

            if( registerBranch.indexOf( item.name ) !== -1 ){
                if( isOrigClass ) continue;
                else jz.log.err( 'jz_gardener_conspecificBranch', item.name );
            }

            let objRequireInfo = jzRequire( isOrigClass ? item.name : orig.path.join( strDirName, item.path ) );
