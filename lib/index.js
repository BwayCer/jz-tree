...
module.exports = new function jzTree(){
     ...
     * @return {Object} :
          @param {Boolean} NativeModule : 是否為原生模組
          @param {Boolean} origClass : 是否為 orig 類型
    ...
    function require_pathAutoSearch( strFileName, strDirReferencePoint ){
        let funcSearchPath = require_pathAutoSearch.searchPath,
            completePath = /^\.{1,2}\//.test( strFileName ) ? strDirReferencePoint + '/' + strFileName : strFileName,
        ...
        if( class_ofOrig ){
            ...
            for(let p = 0, len = nodeModulePaths.length; p < len ; p++){
                anyCompletePath = funcSearchPath( nodeModulePaths[ p ] + '/' + strFileName );
        ...
        return {
            NativeModule: !anyCompletePath,
            origClass: class_ofOrig,
    ...
    function jzRequire( strFilename, strDirReferencePoint ){
        ...
        if( moduleInfo.NativeModule || moduleInfo.origClass ){
            jz.log.remind( 'jz_require_toOrig', strFilename );
            objAns.NativeModule = moduleInfo.NativeModule;
            objAns.origClass = moduleInfo.origClass;
    ...
    jzRequire.vm = function( strFileName, strCode ){
        ...
        jz.require = function( strFilename ){
            return jzRequire( strFilename, strDirName );
        };
