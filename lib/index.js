/*! jzTree - Bway.Cer CC-BY-4.0 @license: bwaycer.github.io/license/CC-BY-4.0.html */
/* 讀者樹 */

"use strict";

//>> 初始化 ------

console.log( "//>> jzTree Initialize -----" );

module.exports = new function jzTree(){
    let jz = this,
        orig = {
            fs: require('fs'),
            path: require('path'),
            module: require('module'),
            vm: require('vm'),
        },
        my = {};


    //>> 暫時控制台 -----

    jz.log = {
        remind: function( strMsgCode, anyAddition ){
            console.log( '@: jzRequire file -', anyAddition );
        },
    };


    //>> 載入模組 -----

    jz.requirePrototype = {};

    /* 載入運行
     * @param {String} fileName : 程式檔路徑
     * @param {String} dirReferencePoint : 參考起始資料夾路徑
     * @return {Object} :
          @param {Boolean} isNativeModule : 是否為原生模組
          @param {Boolean} isOrigClass : 是否為 orig 類型
          @param {String} filename : 程式檔完整路徑
          @param {any} exports : module.exports 的值
     */
    jz.requirePrototype.run = function( strFileName, strDirReferencePoint ){
        let moduleInfo = this.searchPath( strFileName, strDirReferencePoint ),
            objAns = {
                isNativeModule: false,
                isOrigClass: false,
                filename: moduleInfo.filename,
                exports: null
            };

        if( moduleInfo.isJSON ){
            jz.log.remind( 'jz_require_file', objAns.filename );
            objAns.exports = this.readJSON( objAns.filename );
        }else if( moduleInfo.isNativeModule || moduleInfo.isOrigClass ){
            jz.log.remind( 'jz_require_toOrig', objAns.filename );
            objAns.isNativeModule = moduleInfo.isNativeModule;
            objAns.isOrigClass = moduleInfo.isOrigClass;
            objAns.exports = require( objAns.filename );
        }else{
            jz.log.remind( 'jz_require_file', objAns.filename );
            objAns.exports = this.vm_forJz(
                objAns.filename,
                orig.fs.readFileSync( objAns.filename, 'utf-8' )
            );
        }

        return objAns;
    };

    /* 讀取 JSON
     * @param {String} fileName : 程式檔路徑
     * @return {JSON}
     */
    jz.requirePrototype.readJSON = function( strFileName ){
        return orig.fs.existsSync( strFileName ) ? JSON.parse( orig.fs.readFileSync( strFileName, 'utf8') ) : null;
    };

    /* 查找檔案完整路徑及判斷載入方法
     * @param {String} fileName : 程式檔路徑
     * @param {String} dirReferencePoint : 參考起始資料夾路徑
     * @return {Object} :
          @param {Boolean} isNativeModule : 是否為原生模組
          @param {Boolean} isOrigClass : 是否為 orig 類型
          @param {Boolean} isJSON : 是否為 JSON 類型
          @param {String} filename : 程式檔完整路徑
     * 函式補充:
          見 http://www.ruanyifeng.com/blog/2015/05/require.html
          orig.module._nodeModulePaths: 取得模組可能路徑位置
              @param {String} from : 參考的目錄路徑起始點 (絕對 相對皆可)
              @return {Array} : 由參考點開始 並沿父層向上的 node_module 目錄路徑
          orig.module._resolveFilename: 判斷檔案是否存在 (含原生模組) 並取得檔案絕對路徑
              @param {String} request : 詢問的檔案路徑 (絕對 相對皆可)
              @param {?} parent
              @param {?} isMain
              @throw if 路徑錯誤
              @return {String} : 由參考點開始 並沿父層向上的 node_module 目錄路徑
          orig.module._resolveLookupPaths
             orig.module._resolveFilename 下的一個函式 原以為如同 orig.module._nodeModulePaths
             但並非如此 也不知如何使用
     */
    jz.requirePrototype.searchPath = function require_pathAutoSearch( strFileName, strDirReferencePoint ){
        let completePath = /^\.{1,2}\//.test( strFileName ) ? orig.path.join( strDirReferencePoint, strFileName ) : strFileName,
            bisNativeModule = false,
            bisClass_ofOrig = false,
            bisJSONFile = false;

        if( orig.path.extname( completePath ) === '.json' ){
            bisJSONFile = true;
        }else{
            let funcSearchPath = this.searchPath_getFileName.bind( this ),
                anyCompletePath = funcSearchPath( completePath );

            bisClass_ofOrig = !anyCompletePath;

            if( bisClass_ofOrig ){
                let nodeModulePaths = orig.module._nodeModulePaths( strDirReferencePoint );
                for(let p = 0, len = nodeModulePaths.length; p < len ; p++){
                    anyCompletePath = funcSearchPath( orig.path.join( nodeModulePaths[ p ], strFileName ) );
                    if( anyCompletePath ) break;
                }
            }

            bisNativeModule = !anyCompletePath;
            strFileName = orig.module._resolveFilename( !!anyCompletePath ? anyCompletePath : strFileName );
        }

        return {
            isNativeModule: bisNativeModule,
            isOrigClass: bisClass_ofOrig,
            isJSON: bisJSONFile,
            filename: strFileName
        };
    };

    /* 取得存在檔案的完整路徑
     * @param {String} fileName : 程式檔路徑
     * @return {String|Boolean} :
          若此路徑存在為 String : 完整路徑
          否則為 Boolean : false
     */
    jz.requirePrototype.searchPath_getFileName = function( strFileName ){
        if( orig.fs.existsSync( strFileName ) && orig.fs.statSync( strFileName ).isDirectory() ){
            let codePackage = this.readJSON( orig.path.join( strFileName, 'package.json' ) ),
                bisHavePackageMain = !!( codePackage && codePackage.main );
            strFileName = orig.path.join( strFileName, bisHavePackageMain ? codePackage.main : 'index.js' );
        }else{
            strFileName = strFileName + '.js';
        }

        return orig.fs.existsSync( strFileName ) ? strFileName : false;
    };

    /* 虛擬環境
     * @param {Object} info :
          @param {String} fileName : 程式檔路徑 或 可辨識檔案的字串
          @param {String} code : 程式碼
          @param {Object} sandbox : 沙盒環境變數
     * @return {any} : module.exports 的值
     */
    jz.requirePrototype.vm = function( objInfo ){
        if( !objInfo.code ) jz.log.throwErr('jz_vm_needCode');

        let fileName = objInfo.fileName;
        if( !fileName ){
            jz.log.err('jz_vm_needFileName');
            fileName = 'notFileName';
        }

        let objSandbox = this.vm_createSandbox( fileName, objInfo.sandbox );

        orig.vm.runInContext( objInfo.code, objSandbox, {
            filename: fileName,
            displayErrors: true,
            timeout: 1000,
        } );

        return objSandbox.module.exports;
    };

    /* 創建沙盒
     * @param {String} fileName : 程式檔路徑
     * @param {Object} seed : 創建沙盒的種子
          @param {any} 沙盒全域參數[ ...] : 參數物件
     * @return {Object} : orig.vm.createContext 沙盒化的 Object
     */
    jz.requirePrototype.vm_createSandbox = function( strFileName, objSeed ){
        let vmModule = { exports: {} },
            strDirname = orig.path.dirname( strFileName ),
            sandbox = Object.setPrototypeOf( {
                module: vmModule,
                exports: vmModule.exports,
                require: function( strFilename ){
                    strFilename = require_pathAutoSearch( strFilename, strDirname ).filename;
                    jz.log.remind( 'jz_require_fromNative', strFilename );
                    return require( strFilename );
                },
                __dirname: strDirname,
                __filename: strFileName
            }, global );

        for(let key in objSeed ) sandbox[ key ] = objSeed[ key ];

        return orig.vm.createContext( sandbox );
    };

    /* jz 虛擬環境
     * @param {String} fileName : 程式檔路徑
     * @param {String} code : 程式碼
     * @return {any} : strCode 中 module.exports 的值
     */
    jz.requirePrototype.vm_forJz = function( strFileName, strCode ){
        let self = this,
            strDirName = orig.path.dirname( strFileName ),
            jzRequire = function( strFilename ){
                return self.run( strFilename, strDirName );
            };

        return this.vm({
            fileName: strFileName,
            code: strCode,
            sandbox: {
                jz: jz,
                jzRequire: jzRequire,
                orig: orig,
                my: my,
            }
        });
    };

    /* 虛擬環境開放使用
     * 見 jz.requirePrototype.vm
     */
    jz.vm = function( objInfo ){
        this.requirePrototype.vm( objInfo );
    };

    //>> 初次種植 -----

    //第一次的完整路徑是必要的
    jz.requirePrototype.run( __dirname + '/rootearth/jzPlant' );
    jz.gardener.plant( orig.path.join( __dirname, 'growingPoint.json' ) );
}();

